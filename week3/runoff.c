...
bool vote(int voter, int rank, string name)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        {
            // store voter preference
            preferences[voter][rank] = i;
            return true;
        }
    }
    // error if candidate name not found
    return false;
}
...
void tabulate(void)
{
    ...
    for (int v = 0; v < voter_count; v++)
    {
        ...
        for (int c = 0; c < candidate_count; c++)
        {
            ...
            {
                // if candidate not eliminated increase vote count
...
int find_min(void)
{
    // initialize smallest var
    int smallest = MAX_VOTERS;
    // find lowest votes of non-eliminated candidates
    for (int m = 0; m < candidate_count; m++)
    {
        if (!candidates[m].eliminated)
        {
            if (candidates[m].votes < smallest)
            {
                smallest = candidates[m].votes;
            }
        }
    }
    return smallest;
...
bool is_tie(int min)
{
    // check if remaining candidates have the same number of votes
...
void eliminate(int min)
{
    // check which remaining candidate has the lowest number of votes
